...
<head>
  ...
  function cadastrarprova() {
    ...
    form += '<input type="text" id= "ap" placeholder="Assunto da prova" name="assuntoprova" required>'
    form += '<br>';
    form += '<br>';
    for(var i = 1; i <= document.getElementById('qtd_questoes').value; i++){
      form += i + ')<textarea type="text" rows: "4" id= "enun" name="enunciado_' + i + '" placeholder="Enunciado questão" required></textarea>';
      form += '<br>';
      form += '<br>';
      form+= '<div class="form-group row">';
      form+= '<label for="inputEmail3" class="col-sm-2 col-form-label">A)</label>';
      form+= '<div class="col-sm-10">';
      form+= '<textarea type="text" id= "op1" name="opcao1_' + i + '" ' + ' required></textarea>';
      form+= '</div>';
      form+= '</div>';
      form+= '<div class="form-group row">';
      form+= '<label for="inputEmail4" class= "col-sm-2 col-form-label">B)</label>';
      form+= '<div class="col-sm-10">';
      form+= '<textarea type="text" id= "op1" name="opcao2_' + i + '" ' + ' required></textarea>';
      form+= '</div>';
      form+= '</div>';
      form+= '<div class="form-group row">';
      form+= '<label for="inputEmail4" class= "col-sm-2 col-form-label">C)</label>';
      form+= '<div class="col-sm-10">';
      form+= '<textarea type="text" id= "op1" name="opcao3_' + i + '" ' + ' required></textarea>';
      form+= '</div>';
      form+= '</div>';
      form+= '<div class="form-group row">';
      form+= '<label for="inputEmail4" class= "col-sm-2 col-form-label">D)</label >';
      form+= '<div class="col-sm-10">';
      form+= '<textarea type="text" id= "op1" name="opcao4_' + i + '" ' + ' required></textarea>';
      form+= '</div>';
      form+= '</div>';
      form += '<div id= "altc" > Alternativa correta <input type="text" id= "altcorrect" name="correta_' + i + '" placeholder="Alternativa correta" required></div>';
      form += '<br>';
      form += '<div id= "pq" > Peso da questão <input type="text" id= "pquestao" name="pesoquestao_' + i + '" placeholder="Peso da questão" required></div>';
